...
> you need), the assistant now has all 9 schemas in front of it at once — nothing here tells it
> which one applies. **Say explicitly which report type you're generating** (e.g. "generate the
> JSON for the Weekend Report, section 8 — use only that schema, ignore the other eight") before
...
Publishing is the same last step for all nine: copy the JSON block Claude generates, go to `/admin`, log in, pick the matching report type from **Tipo de reporte**, paste the JSON, click **Publicar reporte**. It appears immediately on that report's dashboard page.
