...
> you need), the assistant now has all 10 schemas in front of it at once — nothing here tells it
> which one applies. **Say explicitly which report type you're generating** (e.g. "generate the
> JSON for the Weekend Report, section 8 — use only that schema, ignore the other nine") before
...
9. [CDR Conversion Tracker](#9-cdr-conversion-tracker) — [`docs/CLAUDE_CDR_CONVERSION_TRACKER_PROMPT.md`](CLAUDE_CDR_CONVERSION_TRACKER_PROMPT.md)
10. [CDR Turnover Report](#10-cdr-turnover-report) — [`docs/CLAUDE_CDR_TURNOVER_PROMPT.md`](CLAUDE_CDR_TURNOVER_PROMPT.md)

Publishing is the same last step for all ten: copy the JSON block Claude generates, go to `/admin`, log in, pick the matching report type from **Tipo de reporte**, paste the JSON, click **Publicar reporte**. It appears immediately on that report's dashboard page.
