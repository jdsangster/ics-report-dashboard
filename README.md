...
Claude chat/project (existing workflow)
  ...
  - ALSO generates a JSON block matching that report type's schema
    (prompt templates: docs/CLAUDE_ICS_REPORT_PROMPT.md for ICS,
     ...
     docs/CLAUDE_CDR_TURNOVER_PROMPT.md for the CDR Turnover Report — this one is a
     running log updated as changes happen, not a scheduled publish)
        │
        ▼
...
app/
  ...
  reports/
    ...
    cdr-turnover/page.tsx           CDR Turnover Report (offboardings, resignations, promotions) dashboard (live)
...
lib/
  ...
  turnoverMockData.ts                Simulated CDR Turnover Report for demo mode
...
docs/
  ...
  CLAUDE_CDR_TURNOVER_PROMPT.md      Prompt template for appending an offboarding/resignation/
                                      promotion to the CDR Turnover Report's running log
  ALL_REPORT_PROMPTS.md              All ten prompt templates above combined into one file
```
...
## Report JSON schema (IC Inconsistency)

Full field-by-field reference and a real example: [`docs/CLAUDE_IC_INCONSISTENCY_PROMPT.md`](docs/CLAUDE_IC_INCONSISTENCY_PROMPT.md). This is a discrepancy log, not a full roster — top-level shape: `metadata`, `contributors` (only contributors with an actual BI/Excel mismatch, each with `biCount`, `excelCount`, and an `issues` array of plain-sentence case notes describing the disposition/logging error), and an optional `narrative` closing summary.
...
6. It appears immediately at `/reports/ics`, `/reports/sf-weekly`, `/reports/total-calls`, `/reports/weekend-report`, `/reports/cl-case-review`, `/reports/ic-show-up-rate`, `/reports/ic-inconsistency`, `/reports/operational-complaints`, `/reports/conversion-tracker`, or `/reports/cdr-turnover`.
