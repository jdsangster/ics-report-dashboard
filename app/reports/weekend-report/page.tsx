...
import { WeekendData, Cadence } from "@/lib/types";
import ReportHeader from "@/components/ReportHeader";
import WeekendSummaryCard from "@/components/WeekendSummaryCard";
import CallsTakeawaysSection from "@/components/CallsTakeawaysSection";
import WeekendTeamRankingCard from "@/components/WeekendTeamRankingCard";
import TeamWeekendCard from "@/components/TeamWeekendCard";
import WeekendTopPerformersCard from "@/components/WeekendTopPerformersCard";
import WeekendAttentionCard from "@/components/WeekendAttentionCard";
import ConclusionCard from "@/components/ConclusionCard";

export default function WeekendReportPage() {
  const [reports, setReports] = useState<WeekendData[]>([]);
  ...
  useEffect(() => {
    let cancelled = false;
    fetch("/api/reports?type=weekend-report")
      .then((res) => res.json())
      .then((data: { source: "mock" | "live"; reports: WeekendData[] }) => {
        if (cancelled) return;
  ...
  return (
    <div className="flex flex-1 flex-col">
      <ReportHeader
        ...
        subtitle="Weekend IC Report · Executive Summary"
      />
      ...
      <main className="mx-auto w-full max-w-7xl flex-1 space-y-8 px-6 py-8">
        ...
        >
          <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="text-xl font-semibold tracking-tight text-foreground">
                Weekend IC Report · {activeReport.metadata.periodLabel}
              </h2>
          ...
          <WeekendSummaryCard report={activeReport} />
          <CallsTakeawaysSection takeaways={activeReport.keyTakeaways} />
          <WeekendTeamRankingCard ranking={activeReport.teamRanking} />

          <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
            {activeReport.teams.map((team, i) => (
              <TeamWeekendCard key={team.team} team={team} index={i} />
            ))}
          </div>

          <WeekendTopPerformersCard performers={activeReport.topPerformers} />
          <WeekendAttentionCard
            attentionByTeam={activeReport.attentionByTeam}
            days={activeReport.metadata.days}
            benchmarkPerDay={activeReport.summary.benchmarkPerDay}
          />
