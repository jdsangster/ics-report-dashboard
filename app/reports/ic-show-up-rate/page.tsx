...
import { ICSRatioData, Cadence } from "@/lib/types";
import ReportHeader from "@/components/ReportHeader";
import ICSRatioSummaryCard from "@/components/ICSRatioSummaryCard";
import ICSRatioTierCard from "@/components/ICSRatioTierCard";
import ICSRatioSnapshotCard from "@/components/ICSRatioSnapshotCard";
import ConclusionCard from "@/components/ConclusionCard";

export default function ICShowUpRatePage() {
  const [reports, setReports] = useState<ICSRatioData[]>([]);
  ...
  useEffect(() => {
    let cancelled = false;
    fetch("/api/reports?type=ic-show-up-rate")
      .then((res) => res.json())
      .then((data: { source: "mock" | "live"; reports: ICSRatioData[] }) => {
        if (cancelled) return;
  ...
  return (
    <div className="flex flex-1 flex-col">
      <ReportHeader
        ...
        title="IC and Show Up Rate"
        subtitle="ICS Ratio Ranking · Post-Qualification Conversion"
      />
      ...
      <main className="mx-auto w-full max-w-7xl flex-1 space-y-8 px-6 py-8">
        ...
        >
          <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="text-xl font-semibold tracking-tight text-foreground">
                ICS Ratio Ranking · {activeReport.metadata.periodLabel}
              </h2>
              <p className="text-xs text-muted">
                CDRs ranked by post-qualification conversion rate, grouped into five
                performance tiers.
              </p>
            </div>
            {source === "mock" && (
          ...
          <ICSRatioSummaryCard tiers={activeReport.tiers} description={activeReport.summary.description} />

          <div className="space-y-6">
            {activeReport.tiers.map((tier) => (
              <ICSRatioTierCard key={tier.key} tier={tier} />
            ))}
          </div>

          <ICSRatioSnapshotCard items={activeReport.summary.teamSnapshot} />

          <ConclusionCard conclusion={activeReport.narrative} title="Closing Message" />
        </motion.div>
      </main>
