...
import CSSRankingCard from "@/components/CSSRankingCard";
import CSSConclusionCard from "@/components/CSSConclusionCard";

export default function OperationalComplaintsPage() {
  const [reports, setReports] = useState<CSSData[]>([]);
  ...
  useEffect(() => {
    let cancelled = false;
    fetch("/api/reports?type=operational-complaints")
      .then((res) => res.json())
      .then((data: { source: "mock" | "live"; reports: CSSData[] }) => {
        if (cancelled) return;
  ...
  return (
    <div className="flex flex-1 flex-col">
      <ReportHeader
        ...
        title="Operational Complaint Analysis (CSS)"
        subtitle="CSS Analysis Report · CDR Team"
      />
      ...
      <main className="mx-auto w-full max-w-7xl flex-1 space-y-8 px-6 py-8">
        ...
        >
          <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="text-xl font-semibold tracking-tight text-foreground">
                CSS Analysis Report (CDR Team) · {activeReport.metadata.periodLabel}
              </h2>
              <p className="text-xs text-muted">
                Complaint volume, category distribution, and CDR ranking for the reporting period.
              </p>
            </div>
            {source === "mock" && (
          ...
          <CSSSecondaryCategoriesCard secondaryCategories={activeReport.secondaryCategories} />
          <CSSRankingCard ranking={activeReport.ranking} />
          <CSSConclusionCard conclusion={activeReport.conclusion} />
        </motion.div>
      </main>
